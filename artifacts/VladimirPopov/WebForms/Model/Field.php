        ...
        switch ($this->getType()) {
            ...
            case 'image':
                ...
                if (!empty($file['error']) && $file['error'] == UPLOAD_ERR_INI_SIZE) {
                    $errors[] = __('Uploaded file %s exceeds allowed limit: %s', $file['name'], ini_get('upload_max_filesize'));
                }
                if (isset($file['name']) && file_exists($file['tmp_name'])) {
                    ...
                    if ($this->getType() == 'image') {
                        ...
                        if ($filesize > $images_upload_limit && $images_upload_limit > 0) {
                            $errors[] = __('Uploaded image %s (%s kB) exceeds allowed limit: %s kB', $file['name'], $filesize, $images_upload_limit);
                        }

                        // check that file is valid image
                        if (!@getimagesize($file['tmp_name'])) {
                            $errors[] = __('Unsupported image compression: %s', $file['name']);
                        }

                    } else {
                        // check file size
                        if ($filesize > $files_upload_limit && $files_upload_limit > 0) {
                            $errors[] = __('Uploaded file %s (%s kB) exceeds allowed limit: %s kB', $file['name'], $filesize, $files_upload_limit);
                        }
                    ...
                    if (count($allowed_extensions)) {
                        ...
                        if (!in_array($file_ext, $allowed_extensions)) {
                            $errors[] = __('Uploaded file %s has none of allowed extensions: %s', $file['name'], implode(', ', $allowed_extensions));
                        }
                    }
            ...
            case 'date':
                ...
                if ($customer_value) {
                    ...
                    if(is_array($customer_value))
                        $config['customer_value'] = $customer_value[0];
                }
                $timezone = \Magento\Framework\App\ObjectManager::getInstance()->get('\Magento\Framework\Stdlib\DateTime\Timezone');
