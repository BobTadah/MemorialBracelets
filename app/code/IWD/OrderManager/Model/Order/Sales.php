...
class Sales extends AbstractModel
{
    ...
    private function updateInvoices()
    {
        if ($this->getOrder()->hasInvoices()) {
            if ($this->isOrderTotalIncreased() && $this->allowKeepPrevInvoice()) {
                ...
                $this->createInvoiceForOrder();
                $this->updateShippingAmountForInvoice();
            }
             //fix async grid
            $this->gridPool->refreshByOrderId($this->getOrder()->getId());
        }
    }


    private function updateShippingAmountForInvoice()
    {
        $order = $this->getOrder();
    ...
    private function removeAllInvoices()
    {
        ...
        foreach ($this->getOrder()->getAllItems() as $item) {
            ...
            $item->setDiscountTaxCompensationInvoiced(0);
            $item->setBaseDiscountTaxCompensationInvoiced(0);
...
//            ->save();
        ...
        $order->setDiscountTaxCompensationInvoiced(0);
        $order->setBaseDiscountTaxCompensationInvoiced(0 );
