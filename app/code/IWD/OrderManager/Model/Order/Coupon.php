...
class Coupon extends Order
{
    ...
    private function applyCoupon($couponCode)
    {
        ...
        $itemsCount = $this->getQuote(true)->getItemsCount();
        if ($itemsCount) {
    ...
    protected function prepareItemsParams()
    {
        ...
        foreach ($orderItems as $orderItem) {
            ...
            $orderItemIdsQtys[$quoteItemId] = $orderItem->getQtyOrdered();
            $orderItemIdsPrices[$quoteItemId] = $orderItem->getPrice();
        }

        /** @var \Magento\Quote\Model\Quote\Item $quoteItem */
        foreach ($quoteItems as $quoteItem) {
            $quoteItemId = $quoteItem->getItemId();
            foreach ($orderItemIdsQtys as $key => $value){
                if ($key == $quoteItemId){
                    $quoteItem->setQty($value);
                    $quoteItem->save();
                }
            }
            foreach ($orderItemIdsPrices as $key => $value){
                if ($key == $quoteItemId){
        ...
        foreach ($orderItems as $orderItem) {
            ...
            $quoteItemId = $orderItem->getQuoteItemId();
            if ($quoteItemId != $orderItemId && isset($result[$quoteItemId])) {
                $result[(string)$orderItemId] = $result[$quoteItemId];
