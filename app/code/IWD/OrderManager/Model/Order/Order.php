...
class Order extends \Magento\Sales\Model\Order
{
    ...
    /**
     * @param bool $isApplyCoupon
     *
     * @return $this
     */
    public function syncQuote($isApplyCoupon = false)
    {
        try {
            $this->registry->register('validation_disabled', true);
            ...
            $this->registry->unregister('validation_disabled');
        } catch (\Exception $e) {
    ...
    protected function syncQuoteItems()
    {
        ...
        try {
            ...
            if (count($quoteItemIds) > 0 && $wasRestored) {
                ...
                foreach ($quoteItems as $quoteItem) {
                    try {
                        ...
                        if (!in_array($quoteItemId, $quoteItemIds)) {
                            ...
                            $buyRequest = empty($buyRequest) ? [] : $buyRequest;
                            $buyRequest['qty'] = $qty;
                            $buyRequest['custom_price'] = $orderItem->getPrice();
