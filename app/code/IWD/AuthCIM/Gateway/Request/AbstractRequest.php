...
class AbstractRequest
{
    ...
    {
        $result = null;
        if ($this->config->getSendShippingAddress() && $this->getOrderAdapter()->getIsVirtual() == false) {
            $address = $this->getOrderAdapter()->getShippingAddress();
            ...
            $result['address'] = $address->getStreetLine1() . $address->getStreetLine2();
    ...
    {
        $transId = $this->getPayment()->getTransId();
        $lastTransId = $this->getPayment()->getLastTransId();

        return !empty($transId) ? $transId : $lastTransId;
    }
