...
class ParseResponse
{
     ...
     * @return bool
     */
    public function isDeclined($response)
    {
        if (isset($response['transactionResponse']['errors']['error']['errorText']) && $response['transactionResponse']['errors']['error']['errorText'] != '') {
            return true;
        }else {
            return false;
        }
    ...
    {
        return isset($response['messages']['resultCode']) && $response['messages']['resultCode'] == 'Ok';
    }
    ...
    public function getErrorMessage($response)
    {
        ...
        if (isset($response['transactionResponse']['errors']['error']['errorText'])) {
            $message = '';
            $error = $response['transactionResponse']['errors']['error'];
            if (isset($error['errorCode'])) {
                $message .= '(' . $error['errorCode'] . ') ';
            }
            if (isset($error['errorText'])) {
