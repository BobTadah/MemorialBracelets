...
class UpgradeSchema implements UpgradeSchemaInterface
{
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '1.2.1') < 0) {
            $attachedCustomerTable = $installer->getTable(\IWD\SalesRep\Model\ResourceModel\Customer::TABLE_NAME);
            $ordersTable = $installer->getTable(\IWD\SalesRep\Model\ResourceModel\Order::TABLE_NAME);
            $salesrepUserTable = $installer->getTable(\IWD\SalesRep\Model\ResourceModel\User::TABLE_NAME);

            $installer->getConnection()->addColumn(
                $attachedCustomerTable,
                AttachedCustomer::COMMISSION_TIERED_OPTIONS,
                [
                    'type' => Table::TYPE_TEXT,
                    'default' => null,
                    'comment' => 'sales rep commission tiered options',
                ]
            );
            $installer->getConnection()->addColumn(
                $ordersTable,
            ...
            $installer->getConnection()->addColumn(
                $salesrepUserTable,
                \IWD\SalesRep\Api\Data\UserInterface::BONUS_COMMISSION,
                [
                    'type' => Table::TYPE_DECIMAL,
                    'scale' => 2,
                    'default' => 0,
                    'nullable' => false,
                    'comment' => 'sales rep user bonus commission',
                ]
            );
        }
    }
