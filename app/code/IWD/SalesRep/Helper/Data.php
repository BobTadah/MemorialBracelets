...
class Data extends AbstractHelper
{
    ...
    const XML_PATH_BONUS_NEW_CUSTOMER = 'iwd_salesrep/bonus/new_customer_commission_rate';
    ...
    public function getBonusCommissionConfig()
    {
        return $this->scopeConfig->getValue(self::XML_PATH_BONUS_NEW_CUSTOMER);
    }
    ...
    public function getSalesrepCustomerRole()
    {
        try {
            if ($this->isWithB2B()) {
                $b2bHelper = $this->createB2BClass('\IWD\B2B\Helper\Data');
    ...
    public function saveB2BCustomer($customerId, $adminUser, $data, $salesrepUserId)
    {
        ...
        $this->saveBtbActive($adminUser->getIsActive(), $customerModel->getId(), $isNew);
    }

    private function saveBtbActive($btbActive, $customerId, $isNew = false)
    {
        if ($this->isWithB2B()) {
            if ($isNew) {
                $customer = $this->createB2BClass("\IWD\B2B\Model\Customer");
                $customer->setData('customer_id', $customerId)
                    ->setData('btb_active', $btbActive)
                    ->save();
            } else {
                $b2bCustomer = $this->createB2BClass("\IWD\B2B\Model\ResourceModel\Customer\Collection");
                $companyUsers = $b2bCustomer
                    ->addFieldToFilter('customer_id', $customerId)
                    ->getItems();

                foreach ($companyUsers as $customer) {
                    $customer->setData('btb_active', $btbActive);
                    $customer->save();
                }
            }
        }
     ...
     * @return mixed
     */
    public function createB2BClass($class)
    {
