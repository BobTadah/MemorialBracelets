...
use IWD\OrderGrid\Model\Shipment\Shipment;
...
/**
 * Class Delete
 * @package IWD\OrderGrid\Controller\Adminhtml\Shipment\Massactions
 */
class Delete extends AbstractMassAction
{
     ...
     * {@inheritdoc}
     */
    const ADMIN_RESOURCE = 'IWD_OrderGrid::iwdordergrid_delete_shipment';
    ...
    protected function massAction(AbstractCollection $collection)
    {
        ...
        if ($countNonDeletedShipments && $countDeletedShipments) {
            $this->messageManager->addErrorMessage(
                __('Shipment %1 could not be deleted as deletion of shipments is not permitted. You may enable this option in the Order Grid settings.', $countNonDeletedShipments)
            );
        } elseif ($countNonDeletedShipments) {
            $this->messageManager->addErrorMessage(
                __('Shipment could not be deleted as deletion of shipments is not permitted. You may enable this option in the Order Grid settings.')
            );
        }
