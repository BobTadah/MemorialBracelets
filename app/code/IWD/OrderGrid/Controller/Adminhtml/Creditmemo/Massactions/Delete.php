...
namespace IWD\OrderGrid\Controller\Adminhtml\Creditmemo\Massactions;

use IWD\OrderGrid\Model\Creditmemo\Creditmemo;
...
/**
 * Class Delete
 * @package IWD\OrderGrid\Controller\Adminhtml\Creditmemo\Massactions
 */
    ...
    protected function massAction(AbstractCollection $collection)
    {
        ...
        if ($countNonCancelOrder && $countDeletedCreditMemos) {
            $this->messageManager->addErrorMessage(
                __('Credit memo %1 could not be deleted as deletion of credit memos is not permitted. You may enable this option in the Order Grid settings.', $countNonCancelOrder)
            );
        } elseif ($countNonCancelOrder) {
            $this->messageManager->addErrorMessage(
                __('Credit memo could not be deleted as deletion of credit memos is not permitted. You may enable this option in the Order Grid settings.')
            );
        }
