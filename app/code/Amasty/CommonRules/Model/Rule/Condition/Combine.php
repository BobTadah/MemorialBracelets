...
class Combine extends \Magento\SalesRule\Model\Rule\Condition\Combine
{
    ...
    public function getNewChildSelectOptions()
    {
        ...
        if (!$this->moduleManager->isEnabled(RegistryConstants::AMASTY_SPECIAL_PROMOTIONS_PRO_MODULE_NAME)) {
            $conditions = $this->addAdditionalHandlers($conditions);
        }
     ...
     * @return array
     */
    protected function addAdditionalHandlers(&$conditions)
    {
        $handlerTypes = [
            HandleFactoryInterface::CUSTOMER_HANDLE,
            HandleFactoryInterface::ORDERS_HANDLE
        ];

        $combineHandlerTypes = [
            HandleFactoryInterface::TOTAL_COMBINE_HANDLE
        ];

        foreach ($handlerTypes as $type) {
            $conditions = array_merge_recursive($conditions, $this->handleFactory->create($type));
        }

        foreach ($combineHandlerTypes as $type) {
            $conditions = array_merge_recursive($conditions, $this->combineHandleFactory->create($type));
        }
