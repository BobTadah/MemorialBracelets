            ...
            _totalBaseCalculation: function (input) {
                ...
                switch (this.getCalculationSequence()) {
                    ...
                    case this.CALC_TAX_AFTER_DISCOUNT_ON_INCL:
                        ...
                        //This prevents unnecessary and error prone tax calculations.
                        let currentTaxAmount   = parseFloat($(input.tax_amount.siblings()[0]).text().replace(/[$,]+/g,""));
                        let currentPrice       = parseFloat($(input.price.siblings()[0]).find("b").text().replace(/[$,]+/g,""));
                        let currentTaxPercent  = parseFloat($(input.tax_percent.siblings()[0]).text().replace(/[$,]+/g,""));

                        //Only trigger calculation if actually needs to.
                        if (price !== currentPrice
                            && input.tax_percent.val() !== currentTaxPercent
                        ) {
